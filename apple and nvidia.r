...
symbols <- c("ADBE", "AMD", "MSFT", "QCOM", "EBAY", "ORCL", "CSCO", "INTU", "AAPL", "NVDA")
getSymbols(symbols)
...
# Apple (AAPL) - New
aapl_returns <- dailyReturn(AAPL$AAPL.Close) * 100
basicStats(aapl_returns)
log_aapl_returns <- log(aapl_returns + 1)
basicStats(log_aapl_returns)

# Nvidia (NVDA) - New
nvda_returns <- dailyReturn(NVDA$NVDA.Close) * 100
basicStats(nvda_returns)
log_nvda_returns <- log(nvda_returns + 1)
...
normalTest(log_nvda_returns, method = "jb") 
